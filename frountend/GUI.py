...
old_chat_message = ""
TempDirPath = rf"{current_dir}\frountend\Files"
GraphicsDirPath = rf"{current_dir}\frountend\Graphics"
...
def QueryModifier(Query):
    ...
    question_words = ["how", "what", "who", "where", "when", "why", "which", "whose", "whom", "can you", "what's", "where's"]
...
# Correcting method names with spaces to valid names
...
class ChatSection(QWidget):
    def __init__(self):
        ...
        # GIF on Chat Screen
        movie = QMovie(GraphicsDirectoryPath('GENE.gif'))
        max_gif_size_w = 350
        max_gif_size_h = 350
        ...
        self.text_label.setStyleSheet("color: white; font-size: 16px; margin-right: 195px; border: none; margin-top: -30px;")
    ...
    def SpeechRecogText(self):
        with open(TempDirectoryPath('Status.data'), "r", encoding='utf-8') as file:
            messages = file.read()
            self.text_label.setText(messages)
...
class InitialScreen(QWidget):
    def __init__(self, parent=None):
        ...
        # GIF size on intial screen
        max_gif_size_H = int(screen_width / 16 * 6)
        max_gif_size_W = int(screen_width / 16 * 6)
        movie.setScaledSize(QSize(max_gif_size_W, max_gif_size_H))
        ...
        self.toogled = False
        self.toogle_icon()
        self.icon_label.mousePressEvent = self.toogle_icon
        self.label = QLabel("")
